...
Go also serves the operator API. `services/api-go` exposes asset latest state, alerts, and site rollups over HTTP so the control room and operators can see what the engine has recorded. GridOperator IDs scope organizations and their sites; Actor IDs identify authenticated humans performing audited commands. The API exposes narrow commands such as alert resolution using trusted Actor identity injected by a deployment gateway until built-in authentication exists. Operator commands and engine alert transitions record their corresponding business events in the same transactional outbox. `engine-rust publish-outbox` converts those records to the protobuf topic contract and drains them to Redpanda so stream publication can be retried after commit. Read the endpoint reference and layering in [api.md](api.md).
